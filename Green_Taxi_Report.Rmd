---
title: "Green Taxi Case Study Part1"
...
### Median Trip Distrance by Time of Day for both Weekday and Weekend


From an inital look at the trip distance by time of day graph, The median trip distance is longer at 6 AM and 7 AM than any other time during the day. The difference between the two hours and the rest of the day is quite significant.
...
### Median Trip Distrance by Time of Day for Weekend
...
### Combined Weekday and Weekend Median Trip Distrance
...
To recommend a pick up spot, I leverage the power of unsupervise learning by using a simple Kmeans model to group the pick up spots into 50 groups. Each of the pick up locations
...
According to the dictionary, there are two types of trip - street-hail and dispatch. For this question, we should only focus on the street-hail and exclude the dispatchs. 


```{r}
data %>% select(Trip_type) %>% table()
...
So far, I answered the first three questions. To answer the last question, I would leverage the power of shiny app and make an interactive graph with the input option for longtitude and latitude. Then, I would use the kmeans model to predict which cluster the input location would be in and focus on the pickup points within that cluster. Final, I would pick top 20 pick up points to recommend.

Please found these result from Green Taxi Study Part 2.
